...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    irregMasterTableManager = new TableManager('irreg-master-table', {
        selectable: true,
        idKey: 'Expected3PIrregularID',
        onRowClick: (row, tr) => selectIrregular(row.Expected3PIrregularID, tr),
    ...
    irregDetailTableManager = new TableManager('irreg-detail-table', {
        selectable: true,
        idKey: 'Expected3PIrregularProductID',
...
async function bulkDeleteBaseItems() {
    ...
    showConfirm(`${selectedIds.length}개 항목을 삭제하시겠습니까?`, async () => {
        try {
            const result = await api.post('/api/expected/3p/regular/bulk-delete', { ids: selectedIds });
...
async function loadIrregMaster() {
    try {
        ...
        irregMasterDataMap = {};
        data.forEach(row => irregMasterDataMap[row.Expected3PIrregularID] = row);
...
async function loadIrregDetail(irreg) {
    try {
        ...
        const result = await api.get(`/api/expected/3p/irregular/products?expected_3p_irregular_id=${irreg.Expected3PIrregularID}&page=1&limit=10000`);
        const items = result.data || [];
...
async function bulkDeleteIrregItems() {
    ...
    showConfirm(`${selectedIds.length}개 상품을 삭제하시겠습니까?`, async () => {
        try {
            const result = await api.post('/api/expected/3p/irregular/products/bulk-delete', { ids: selectedIds });
...
async function loadStatuses() {
    try {
        const result = await api.get('/api/expected/3p/irregular/statuses');
...
function downloadTemplate() {
    const endpoint = currentTab === 'base'
        ? '/api/expected/3p/regular/download'
        : '/api/expected/3p/irregular/download';
...
function downloadIrregEditForm() {
    ...
    const params = { ids: selectedIds.join(',') };
    const queryString = api.buildQueryString(params);
    window.location.href = `/api/expected/3p/irregular/download${queryString}`;
}
...
async function uploadFile() {
    ...
    try {
        ...
        const endpoint = currentTab === 'base'
            ...
            : '/api/expected/3p/irregular/upload';

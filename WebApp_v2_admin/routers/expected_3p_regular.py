...
from repositories.expected_3p_regular_repository import Expected3PRegularRepository
...
# ========== 위탁 정기 예상 Router ==========
router = APIRouter(prefix="/api/expected/3p/regular", tags=["Expected3PRegular"])

# Repository 인스턴스
expected_3p_regular_repo = Expected3PRegularRepository()
...
# Pydantic Models - 위탁 정기 예상
class Expected3PRegularCreate(BaseModel):
    Date: str
    BrandID: int
...
class Expected3PRegularUpdate(BaseModel):
...
class Expected3PRegularBulkUpdateItem(BaseModel):
    Expected3PRegularID: int
    ExpectedAmount: Optional[float] = None
    ExpectedQuantity: Optional[int] = None
    Notes: Optional[str] = None


class Expected3PRegularBulkUpdateRequest(BaseModel):
    items: List[Expected3PRegularBulkUpdateItem]
...
async def get_expected_3p_regular_list(
    ...
    try:
        ...
        result = expected_3p_regular_repo.get_list(
            page=page,
            limit=limit,
            filters=filters,
            order_by=order_by,
        ...
        raise HTTPException(500, f"위탁 정기 예상 조회 실패: {str(e)}")
...
async def get_expected_3p_regular_channels(
    ...
    """채널별 위탁 정기 예상 요약 조회 (마스터 패널용)"""
    try:
        if not year_month:
            raise HTTPException(400, "년월은 필수입니다")
        channels = expected_3p_regular_repo.get_channels_summary(year_month, brand_id)
...
async def get_expected_3p_regular_channel_items(
    ...
    """특정 채널의 위탁 정기 예상 상품 목록 조회 (디테일 패널용)"""
    try:
        if not year_month:
            raise HTTPException(400, "년월은 필수입니다")
        items = expected_3p_regular_repo.get_by_channel(channel_id, year_month, brand_id)
...
@log_activity("UPDATE", "Expected3PRegularProduct")
async def bulk_update_expected_3p_regular(
    request_body: Expected3PRegularBulkUpdateRequest,
    request: Request,
    user: CurrentUser = Depends(require_permission("Expected3PRegular", "UPDATE"))
):
    """위탁 정기 예상 인라인 편집 일괄 저장"""
    try:
...
async def download_expected_3p_regular(
    ...
    user: CurrentUser = Depends(require_permission("Expected3PRegular", "EXPORT"))
):
    ...
    try:
        ...
        if ids:
            ...
            data = expected_3p_regular_repo.get_by_ids(id_list)
            ...
            for ch_id in ch_id_list:
                items = expected_3p_regular_repo.get_by_channel(ch_id, year_month, brand_id)
                data.extend(items)
            ...
            result = expected_3p_regular_repo.get_list(page=1, limit=100000, filters=filters)
            data = result['data']
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            df.to_excel(writer, index=False, sheet_name='위탁 정기 예상')
            guide_df.to_excel(writer, index=False, sheet_name='안내')

            workbook = writer.book
            worksheet = writer.sheets['위탁 정기 예상']
        ...
        filename = f"expected_3p_regular_{year_month or 'template'}.xlsx"
        headers = {
...
async def get_expected_3p_regular_item(target_id: int, user: CurrentUser = Depends(require_permission("Expected3PRegular", "READ"))):
    """위탁 정기 예상 단일 조회"""
    try:
        item = expected_3p_regular_repo.get_by_id(target_id)
        if not item:
...
@router.post("")
@log_activity("CREATE", "Expected3PRegularProduct", id_key="Expected3PRegularID")
async def create_expected_3p_regular(
    data: Expected3PRegularCreate,
    request: Request,
    user: CurrentUser = Depends(require_permission("Expected3PRegular", "CREATE"))
):
    """위탁 정기 예상 생성"""
    try:
        target_id = expected_3p_regular_repo.create(data.dict(exclude_none=True), user_id=user.user_id)

        return {"Expected3PRegularID": target_id, "UniqueCode": data.UniqueCode, "Date": data.Date}
    except Exception as e:
        raise HTTPException(500, f"예상 매출 생성 실패: {str(e)}")


@router.put("/{target_id}")
@log_activity("UPDATE", "Expected3PRegularProduct", id_key="Expected3PRegularID")
async def update_expected_3p_regular(
    target_id: int,
    data: Expected3PRegularUpdate,
    request: Request,
    user: CurrentUser = Depends(require_permission("Expected3PRegular", "UPDATE"))
):
    """위탁 정기 예상 수정"""
    try:
        if not expected_3p_regular_repo.exists(target_id):
            raise HTTPException(404, "예상 매출 데이터를 찾을 수 없습니다")
        ...
        success = expected_3p_regular_repo.update(target_id, update_data, user_id=user.user_id)
        if not success:
            raise HTTPException(500, "예상 매출 수정 실패")

        return {"Expected3PRegularID": target_id, **update_data}
    except HTTPException:
        raise
    except Exception as e:
...
@log_delete("Expected3PRegularProduct", id_param="target_id")
async def delete_expected_3p_regular(
    target_id: int,
    request: Request,
    user: CurrentUser = Depends(require_permission("Expected3PRegular", "DELETE"))
):
    """위탁 정기 예상 삭제"""
    try:
        if not expected_3p_regular_repo.exists(target_id):
        ...
        success = expected_3p_regular_repo.delete(target_id)
        if not success:
...
@log_bulk_delete("Expected3PRegularProduct")
async def bulk_delete_expected_3p_regular(
    ...
    try:
        ...
        deleted_count = expected_3p_regular_repo.bulk_delete(request_body.ids)
...
@log_activity("BULK_DELETE", "Expected3PRegularProduct")
async def filter_delete_expected_3p_regular(
    request_body: FilterDeleteRequest,
    request: Request,
    user: CurrentUser = Depends(require_permission("Expected3PRegular", "DELETE"))
):
    """필터 조건으로 위탁 정기 예상 일괄 삭제"""
    try:
        deleted_count = expected_3p_regular_repo.delete_by_filter(
...
async def upload_expected_3p_regular(
    ...
    try:
        ...
        # UPSERT 실행
        result = expected_3p_regular_repo.bulk_upsert(records)
        ...
        if user and request:
            activity_log_repo.log_action(
                ...
                action_type="CREATE",
                target_table="Expected3PRegularProduct",
                details={
                    "action": "EXCEL_UPLOAD",

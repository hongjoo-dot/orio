...
async def get_expected_3p_regular_input_months(
    year_month: Optional[str] = None,
    user: CurrentUser = Depends(require_permission("Expected3PRegular", "READ"))
):
    """위탁 정기 예상 InputMonth(입력월) 목록 조회"""
    try:
        input_months = expected_3p_regular_repo.get_input_months(year_month)
...
async def upload_expected_3p_regular(
    ...
    """위탁 정기 예상 엑셀 업로드 (input_month: 입력월 YYYY-MM)"""
    try:

...
async def get_expected_3p_irregular_year_months(
    input_month: Optional[str] = None,
    user: CurrentUser = Depends(require_permission("Expected3PIrregular", "READ"))
):
    """행사 년월 목록 조회 (입력월 종속 필터)"""
    try:
        year_months = expected_3p_irregular_repo.get_year_months(input_month)
...
async def download_expected_3p_irregulars(
    ...
    try:
        ...
        guide_data = [
            ...
            ['판매가~기타비', '가격/비용 정보'],
            ['예상매출(상품)', 'VAT 포함 금액. VAT제외 금액은 서버에서 자동 계산됩니다.'],
            ['예상수량(상품)', '숫자'],
            ['메모(상품)', '메모'],
            ['', ''],
            ['■ 주의사항', ''],
...
async def get_expected_3p_irregular_product_list(
    ...
    try:
        ...
        if expected_3p_irregular_id:
            filters['expected_3p_irregular_id'] = expected_3p_irregular_id
        if year_month:
            filters['year_month'] = year_month
        if brand_id:
            filters['brand_id'] = _parse_int_list(brand_id)

...
def run_analytics_pipeline(days: int = None) -> dict:
    ...
    logger.info(f'Cafe24 주문별 UTM 파이프라인 시작 (최근 {days}일)')

    try:
        from .main import main as analytics_main
        result = analytics_main(days=days)

        logger.info('Cafe24 주문별 UTM 파이프라인 완료')
        return result

    except Exception as e:
        logger.error(f'Cafe24 주문별 UTM 파이프라인 오류: {str(e)}', exc_info=True)
        raise

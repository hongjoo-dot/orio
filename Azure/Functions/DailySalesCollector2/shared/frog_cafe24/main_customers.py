"""
Frog Cafe24 고객 데이터 수집 파이프라인
수집 → DB(FrogCafe24Customers) → Slack
"""
import os
import logging
...
def main():
    ...
    logging.info("=" * 70)
    logging.info(f"Frog Cafe24 고객 데이터 수집 시작: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logging.info("=" * 70)
    ...
    try:
        ...
        # Step 3: 최종 결과
        logging.info("=" * 70)
        logging.info("Frog Cafe24 고객 데이터 처리 완료!")
        ...
        error_message = f"[ERROR] Frog Cafe24 고객 데이터 수집 실패\n오류: {str(e)}"
